...
const recommendService = {
  ...
  getRecommendations: async (alcohol, bounds) => {
    try {
      ...
      const response = await authApi.get(`/recommend/${alcohol}`, {
        params: {
          minX: bounds.minX,
          minY: bounds.minY,
          maxX: bounds.maxX,
  ...
  getRemainingCount: async () => {
    try {
      ...
      const response = await authApi.get("/recommend/remaining");
  ...
  checkRemainingCount: async () => {
    try {
      // authApi 인터셉터가 Firebase 토큰을 자동으로 헤더에 넣어줍니다.
      const response = await authApi.get("/recommend/check");
